...
DEBUG = True  # Should set to False as default
if DEBUG:
    assert not config.stage_execution.multiprocessing.use
...
def run_program(dataset_function_parameters, timeout, log_to_stdout, function_body_sample):
    ...
    dataset_function_arguments = {key: sample[key] for key in sample if key not in ("id", "auxiliary_string", "answer")}
    ...
    try:
        ...
        print_section('-', f"SAMPLE ID: {sample_id} INVOCATION", f"{filename}.execute_command(**args_dict)",
                      worker_log_path, log_to_stdout)
        ...
        if DEBUG:
            ...
            if thread.is_alive():
                ...
                print_section('!', "TIMEOUT ERROR", f"Sample {sample_id} timed out after {timeout} seconds",
                              worker_log_path, log_to_stdout)
                ...
                print_section('!', "RUNTIME ERROR",
                              f"Sample {sample_id} failed with error: {evalue}. \n Traceback: \n{formatted_tb}",
                              worker_log_path, log_to_stdout)
                result = "error during execution"
...
def stage_generation(stage_generation_config, dataloader, out_dir):
    ...
    dataloader = DataLoader(dataset, batch_size=stage_generation_config.model.batch_size, num_workers=0,
...
def stage_execution(stage_execution_config, dataset, stage_generation_results, out_dir):
    ...
    with open(full_results_json_path, "w") as f_json:
        json.dump(result, f_json, indent=2, ensure_ascii=False,
                  default=str)  # the default is important, for non-serializable types, to just try to force to string.
...
def stage_execution_multiprocessing(stage_execution_config, dataset, stage_generation_results, out_dir):
    all_sample_ids, all_function_bodies = zip(
        *[(id_, value['code']) for id_, value in stage_generation_results.items()])
...
if __name__ == '__main__':
    ...
    if config.stage_execution.enabled:
        if stage_generation_results is None:
            stage_generation_results = deserialize_stage_generation_results(
                config.stage_execution.stage_generation_results_path)

        context.stage = 'execution'
        if not config.stage_execution.multiprocessing.use:
            stage_execution_results = stage_execution(config.stage_execution, dataset, stage_generation_results,
                                                      trial_path)
        else:
            stage_execution_results = stage_execution_multiprocessing(config.stage_execution, dataset,
                                                                      stage_generation_results, trial_path)

    if config.stage_evaluation.enabled:
        if stage_execution_results is None:
            if config.stage_generation.enabled:
                print(
                    "skipping evaluation because it seems generation was done, then execution was skipped. You may have an execution json in config.stage_evaluation.stage_execution_results_path but there isn't much point to this.")
                exit(1)

            stage_execution_results = deserialize_stage_execution_results(
                config.stage_evaluation.stage_execution_results_path)

            ...
            for i, batch in tqdm(enumerate(dataloader), total=n_batches):
                ...
                if config.execute_code:
                    ...
                    warnings.warn("Not executing code! This is only generating the code. We set the flag "
                                  ...
                                  "it.")
                import objgraph
                objgraph.show_backrefs([batch['image']], max_depth=5)
                objgraph.show_backrefs([batch['image'][0]], max_depth=5)

                import pdb; pdb.set_trace()
                del batch['image']  # Remove explicit reference to tensors. batch['image'] is a list of tensors
                import gc
                gc.collect()
